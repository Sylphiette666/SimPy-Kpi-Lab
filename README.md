...
- 可选事件轨迹记录：`experiment.record_traces: true` 时导出 `traces.xes`（流程挖掘标准格式）与 `traces.jsonl`，可与 pm4py 等工具对接。
- 数据驱动的模型挖掘：`simlab mine` 从 XES/JSONL 事件日志估计到达/服务分布、工位并发容量与周期时间目标，自动生成 YAML。
- LLM 蓝图式模型设计：`simlab design` 把自然语言需求描述转成经 Pydantic 校验的仿真配置（OpenAI/DeepSeek 双路径，只输出数据不执行代码）。
- 结果可视化：`simlab plot` 生成 KPI 对比（含置信区间）、工位利用率、轨迹甘特图、模型结构图与 HTML 报告。
...
在配置中开启 `experiment.record_traces: true` 后，仿真逐实体记录事件轨迹（到达、各工位服务开始/结束、完成，含等待与服务时长属性），保存结果时额外导出：

```text
outputs/service_center/
├── traces.xes       # XES 1.0（流程挖掘标准格式，可被 pm4py 等工具直接读取）
└── traces.jsonl     # 每行一个事件的 JSON，便于 pandas / polars 分析
```

轨迹不写入 `results.json`，关闭开关时几乎无额外开销。XES 时间映射约定：仿真时间被当作业务时间单位，以 1970-01-01T00:00:00Z 为基准加偏移形成 `time:timestamp`，原始浮点时间保留在 `simlab:time` 属性中。

流程挖掘方向（可选路线）：用 pm4py 对真实事件日志做过程发现与分布参数估计，反向生成 YAML 配置，实现数据驱动的模型生成。注意 pm4py 为 AGPL-3.0 许可，应作为可选依赖通过 XES 文件交互，不并入本项目的 MIT 核心。

`simlab mine` 内置了轻量挖掘（不依赖 pm4py）：从事件日志估计工位序列、分布参数与并发容量。多场景日志（case id 含 `:r` 前缀）会按场景分组计算容量取众数，并对组间参数差异发出警告；分布拟合按变异系数启发式选择（cv≥0.9 指数 / cv≤0.2 定值 / 其余三角）。

## LLM 蓝图式模型设计

用自然语言直接生成模型蓝图（对应“Model generation”环节），生成的是结构化数据而非代码，且必须通过完整 Pydantic 校验：

```powershell
.\.venv\Scripts\simlab.exe design "设计一个体检中心：前台登记平均3分钟（指数分布），
医生检查三角分布4/8/15分钟，每小时12人到达，营业8小时，2名医生，服务水平目标30分钟" `
  --output outputs/designed_clinic.yaml
```

生成后用 `simlab validate` 校验、`simlab run` 验证后再投入使用。模型名默认跟随 `SIMLAB_OPENAI_MODEL` / `SIMLAB_OPENAI_BASE_URL` 环境变量（DeepSeek 端点自动用 deepseek-chat）。

## 结果可视化

`simlab plot outputs/service_center/results.json` 生成（需 `pip install -e ".[viz]"`）：

```text
outputs/service_center/plots/
├── kpi_comparison.png      # 关键 KPI 场景对比（误差棒为 95% 置信区间）
├── station_utilization.png # 各工位利用率按场景分组
├── traces_gantt.png        # 前若干实体的轨迹甘特图（需 record_traces）
├── model_structure.png     # 模型结构图（到达 → 工位 → 完成）
└── report.html             # KPI 表 + 全部图表的 HTML 报告
```

也可在 `simlab run` 时加 `--plot` 一步完成仿真与出图。
